...
//TODO 4: Add permission "pollmaster.admin" and --override flag
    ...
    public boolean closePoll(final Player whoClicked, final int pollId) {
        ...
        try (Connection connection = new ConnectionManager().getConnection()) {
                ...
                try (ResultSet resultSet = checkCreatorStmt.executeQuery()) {
                    if (resultSet.next()) {
                        ...
                        if (!creatorUUID.equals(whoClicked.getUniqueId().toString())) {
                            whoClicked.sendMessage(ChatColor.RED + LangUtil.getTranslation("close_poll_not_creator"));
                            return false;
                        }
                    } else {
            ...
            // Prepare the SQL statements to Update the poll and associated votes
            String updatePollQuery = "UPDATE polls SET isOpen=0 WHERE id = ?";
            String updateVotesQuery = "UPDATE votes SET isOpen=0 WHERE poll_id = ?";

            // Delete the poll itself
            try (PreparedStatement updatePollStmt = connection.prepareStatement(updatePollQuery)) {
                updatePollStmt.setInt(1, pollId);
                int rowsAffectedPolls = updatePollStmt.executeUpdate();

                // Check if the poll was successfully deleted
                if (rowsAffectedPolls > 0) {
                    // Delete associated votes
                    try (PreparedStatement updateVotesStmt = connection.prepareStatement(updateVotesQuery)) {
                        updateVotesStmt.setInt(1, pollId);
                        updateVotesStmt.executeUpdate(); // Update all votes associated with the poll
                    }

                    whoClicked.sendMessage(ChatColor.GREEN + LangUtil.getTranslation("poll_closed"));
                    return true;
                } else {
                    whoClicked.sendMessage(ChatColor.RED + LangUtil.getTranslation("poll_not_found"));
                    return false;
            ...
            e.printStackTrace();
            whoClicked.sendMessage(ChatColor.RED + LangUtil.getTranslation("poll_close_error"));
        }
    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String s, final String[] args) {
        ...
        if (!player.hasPermission("pollmaster.close")) {
            player.sendMessage(ChatColor.RED + LangUtil.getTranslation("command_no_permission"));
            return true;
        }

        closePoll(player, Integer.parseInt(args[0]));
        return true;
    }
